...
const STATUS_CONFIG = (t) => ({
  en_attente: { label: t('statut_en_attente'), color: '#f59e0b', bg: 'rgba(245,158,11,0.1)', icon: '⏳' },
  en_cours:   { label: t('statut_en_cours'),   color: '#0ea5e9', bg: 'rgba(14,165,233,0.1)', icon: '⚙️' },
  termine:    { label: t('statut_termine'),     color: '#10b981', bg: 'rgba(16,185,129,0.1)', icon: '✅' },
  annule:     { label: t('statut_annule'),      color: '#f43f5e', bg: 'rgba(244,63,94,0.1)',  icon: '✕' },
});
...
function DemandeCard({ demande, onAction, isFournisseur, isAdmin, currentUserId }) {
  ...
  const status = cfg[demande.statut] || cfg.en_attente;
  const date = demande.date_demande || demande.createdAt
    ? new Date(demande.date_demande || demande.createdAt).toLocaleDateString(isAr ? 'ar-MA' : 'fr-FR', { day: '2-digit', month: 'short', year: 'numeric' }) : '—';
  ...
  const handleAction = async (statut, motif = '') => {
    setLoadingAction(statut); await onAction(demande.id, statut, motif);
    setLoadingAction(null); setShowRefuseModal(false);
  };

  return (
    <div style={styles.card} dir={isAr ? 'rtl' : 'ltr'}>
      <div style={styles.cardHeader}>
        <div>
          <span style={styles.cardId}>{isAr ? 'طلب' : 'Demande'} #{demande.id}</span>
          <h3 style={styles.cardTitle}>{demande.Service?.nom || '—'}</h3>
        </div>
        <span style={{ ...styles.badge, color: status.color, background: status.bg }}>{status.icon} {status.label}</span>
      </div>

      <div style={styles.cardMeta}>
        {demande.adresse && <span style={styles.metaItem}>📍 {demande.adresse}</span>}
        <span style={styles.metaItem}>📅 {date}</span>
        {demande.Service?.prix && <span style={{ ...styles.metaItem, color: 'var(--sky)' }}>💰 {demande.Service.prix} {isAr ? 'درهم' : 'MAD'}</span>}
      </div>

      {isAdmin && (
        <div style={styles.infoBlock}>
          <div style={styles.infoRow}>
            <span style={styles.infoLabel}>👤 {t('paiements_client')}</span>
            <span style={styles.infoValue}>{clientNom}</span>
          </div>
          <div style={styles.infoDivider} />
          <div style={styles.infoRow}>
            <span style={styles.infoLabel}>🔧 {isAr ? 'مزود الخدمة المعين' : 'Fournisseur assigné'}</span>
            <span style={{ ...styles.infoValue, color: fournisseurNom ? '#10b981' : '#f59e0b' }}>
              {fournisseurNom || (isAr ? 'غير معين' : 'Non assigné')}
            </span>
          </div>
        </div>
      )}

      {isFournisseur && <div style={styles.clientLine}><span style={styles.metaItem}>👤 {t('paiements_client')} : {clientNom}</span></div>}
      ...
      {isFournisseur && (
        <div style={styles.actions}>
          {demande.statut === 'en_attente' && (
            <>
              <button disabled={loadingAction === 'en_cours'} onClick={() => handleAction('en_cours')} style={styles.acceptBtn}>
                {loadingAction === 'en_cours' ? '⏳' : '✅'} {t('action_accept')}
              </button>
              <button onClick={() => setShowRefuseModal(true)} style={styles.refuseBtn}>✕ {t('action_refuse')}</button>
            </>
          )}
          {demande.statut === 'en_cours' && (
            isAssigned
              ? <button disabled={loadingAction === 'termine'} onClick={() => handleAction('termine')} style={styles.termineBtn}>{loadingAction === 'termine' ? '⏳' : '🏁'} {t('action_termine')}</button>
              : <span style={styles.lockedMsg}>{t('action_locked')}</span>
          )}
        </div>
      )}

      {isAdmin && demande.statut === 'en_attente' && <div style={styles.adminReadOnly}>📋 {isAr ? 'في انتظار مزود خدمة' : "En attente d'un fournisseur"}</div>}
      {isAdmin && demande.statut === 'en_cours' && <div style={styles.adminReadOnly}>⚙️ {isAr ? 'جارٍ — مزود الخدمة:' : 'En cours — fournisseur :'} {fournisseurNom || '—'}</div>}

      {demande.statut === 'annule' && demande.motif_refus && (
        <div style={styles.motifBox}>
          <span style={styles.motifLabel}>{isAr ? 'سبب الرفض:' : 'Motif :'}</span>
      ...
      {showRefuseModal && (
        <div style={styles.miniOverlay} onClick={() => setShowRefuseModal(false)}>
          <div style={styles.miniModal} onClick={e => e.stopPropagation()} dir={isAr ? 'rtl' : 'ltr'}>
            <h3 style={styles.miniTitle}>✕ {t('action_refuse')}</h3>
            <p style={styles.miniSub}>{isAr ? 'حدد السبب (اختياري)' : 'Précisez la raison (optionnel)'}</p>
            <textarea value={refusMotif} onChange={e => setRefusMotif(e.target.value)} rows={3}
              placeholder={isAr ? 'مثال: خارج النطاق، الجدول ممتلئ...' : 'Ex: Hors zone, planning complet...'}
              style={styles.miniTextarea} />
            <div style={styles.miniActions}>
              <button onClick={() => setShowRefuseModal(false)} style={styles.cancelBtn}>{t('cancel')}</button>
              <button onClick={() => handleAction('annule', refusMotif)} disabled={loadingAction === 'annule'} style={styles.confirmRefuseBtn}>
                {loadingAction === 'annule' ? '⏳' : `✕ ${isAr ? 'تأكيد الرفض' : 'Confirmer le refus'}`}
              </button>
            </div>
          </div>
        </div>
...
export default function DemandesATraiter() {
  ...
  const load = async () => {
    setLoading(true);
    try { const res = await demandesAPI.getAll(); setDemandes(Array.isArray(res.data) ? res.data : []); }
  ...
  const handleAction = async (id, statut, motif) => {
    try {
      ...
      const messages = {
        en_cours: isAr ? '✅ تم قبول الطلب.' : '✅ Demande acceptée.',
        termine:  isAr ? '🏁 تم إنهاء الطلب.' : '🏁 Demande terminée.',
        annule:   isAr ? '✕ تم رفض الطلب.'   : '✕ Demande refusée.',
      };
  ...
  const tabs = [
    { value: 'all',        label: t('demandes_all'),     emoji: '📋' },
    { value: 'en_attente', label: t('statut_en_attente'),emoji: '⏳', urgent: count('en_attente') > 0 },
    { value: 'en_cours',   label: t('statut_en_cours'),  emoji: '⚙️' },
    { value: 'termine',    label: t('statut_termine'),   emoji: '✅' },
    { value: 'annule',     label: t('statut_annule'),    emoji: '✕' },
  ];
  ...
  return (
    <div style={styles.page} dir={isAr ? 'rtl' : 'ltr'}>
      <div style={styles.header}>
        <div>
          <h1 style={styles.title}>{isAdmin ? (isAr ? 'جدول الطلبات' : 'Tableau des demandes') : (isAr ? 'الطلبات للمعالجة' : 'Demandes à traiter')}</h1>
          <p style={styles.subtitle}>{demandes.length} {isAr ? 'طلب' : `demande${demandes.length !== 1 ? 's' : ''}`}</p>
        </div>
        <div style={styles.kpis}>
          {[
            { label: t('statut_en_attente'), val: count('en_attente'), color: '#f59e0b' },
            { label: t('statut_en_cours'),   val: count('en_cours'),   color: '#0ea5e9' },
            { label: t('statut_termine'),    val: count('termine'),    color: '#10b981' },
          ].map((k, i, arr) => (
      ...
      <div style={styles.tabs}>
        {tabs.map(tab => (
          <button key={tab.value} onClick={() => setFilterStatut(tab.value)}
            style={{ ...styles.tab, ...(filterStatut === tab.value ? styles.tabActive : {}), ...(tab.urgent ? styles.tabUrgent : {}) }}>
            {tab.emoji} {tab.label}
            <span style={styles.tabCount}>{tab.value === 'all' ? demandes.length : count(tab.value)}</span>
          </button>
        ))}
      </div>

      {loading ? <div style={styles.center}><div className="spinner" /></div>
        : filtered.length === 0 ? <div style={styles.empty}><span style={{ fontSize: 48 }}>📭</span><p>{t('no_data')}</p></div>
        : <div style={styles.list}>{filtered.map(d => <DemandeCard key={d.id} demande={d} onAction={handleAction} isFournisseur={isFournisseur} isAdmin={isAdmin} currentUserId={user?.id} />)}</div>}
    </div>
  );
}
...
const styles = {
  ...
  card: { background: 'rgba(30,41,59,0.7)', border: '1px solid rgba(14,165,233,0.12)', borderRadius: 16, padding: 24 },
  ...
  motifLabel: { fontSize: 11, color: '#f43f5e', textTransform: 'uppercase', marginRight: 8 },
  ...
  miniModal: { background: '#1e293b', border: '1px solid rgba(244,63,94,0.3)', borderRadius: 16, padding: 28, width: '100%', maxWidth: 420 },
